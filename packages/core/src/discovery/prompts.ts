...
4. A value-ranked scenario plan: material user jobs Iris should test, each tied to discovered surface ids.
5. A value-ranked list of testable seed goals derived from selected material scenarios. These are what an autonomous UX evaluator will go verify.
...
- Then learn the product: infer the primary user job, the durable artifact/state/result a real user would expect, and the material scenarios that prove that job. Surfaces inform this map; surfaces are not automatically goals.
- Convert only material core and selected secondary workflow scenarios into seed goals. The number of goals is the result of value ranking, not a target. Never stop because you reached a preferred count, and never pad with invented goals to reach a count.
...
- For artifact editors such as canvas, diagram, whiteboard, document, media, or builder tools, selected seed goals should be dominated by artifact manipulation. The primary must-goal should create a minimally meaningful artifact when the visible tools support it: combine 2-3 normal user operations such as draw/place + label/type + style/move/resize. A word editor should type a real paragraph plus formatting and save/export if available. A single trivial object can be a smoke check, but it should not be the whole primary value-loop proof when richer creation controls are visible.
...
- Donation or upsell prompt: usually setup/sample context. Only make it a goal when the prompt blocks the primary value loop or the donation/upsell flow is itself a central product purpose.
...
- journeys: one scenario per user intent worth testing; reference surface_ids; use priority "must" only for central product outcomes, "should" for important secondary workflows, and "could" for low-priority coverage. Include goal_class.

...
- Goals should usually prove one coherent state transition. Avoid chaining a happy-path state, an edge-case state, and a navigation check into one seed goal; split them or choose the highest-value normal-user proof state. For example, a data grid search goal should prove either a concrete filtered row/count state or a no-match empty state, not both unless the product's main purpose is search-error handling.
- For concrete app/tool products, support documentation, reference tables, related links, header search, examples, PDFs, account links, and help pages are context or optional coverage unless that support surface is the product's primary value loop. Do not let support/reference coverage crowd out primary create/calculate/filter/sort/edit/checkout/artifact goals.
